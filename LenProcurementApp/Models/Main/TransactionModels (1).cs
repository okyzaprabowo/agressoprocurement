    ...
    public class MainTransaction
    {
        ...
        public List<NewMainDashboard> getMainTableContents()
        {
            ...
            var query = " SELECT DISTINCT "
                        ...
                        + " WHERE LEFT(d.tgl_pengajuan, 7) BETWEEN '"+ cDateFrom + "' AND '"+ cDate + "' "
        ...
        public List<ExcelActive> GetExcelActive(string from, string to)
        {
            try
            {
                var query = "SELECT DISTINCT d.user_dpb AS pengguna, ( SELECT COUNT(ex.executor) FROM len_executor ex WHERE ex.dpb = d.dpb ) AS plk, d.dpb, ( SELECT ex.created_at FROM len_executor ex WHERE ex.dpb = d.dpb LIMIT 1 ) AS tgl_masuk, d.divisi_t AS divisi, d.job_code AS kode_proyek, dp.product, d.job_code_t AS nama_proyek, ( SELECT pr.product_t FROM len_enq_product pr WHERE pr.product = dp.product LIMIT 1 ) AS description, dp.account, dp.unit, d.cur, dp.unit_price, dp.qty AS 'order', dp.cur_amount, dp.total_idr, d.tgl_pengajuan, d.tgl_dibutuhkan, b.spph, b.po, ( SELECT p.jenis_order FROM len_enq_po p WHERE b.po = p.po LIMIT 1 ) AS jenis_order, ( SELECT p.supplier_t FROM len_enq_po p WHERE b.po = p.po LIMIT 1 ) AS supplier, dp.qty AS 'order2', ( SELECT pp.cur FROM len_enq_po_product pp WHERE b.po = pp.po LIMIT 1 ) AS cur2, ( SELECT ppp.unit_price FROM len_enq_po_product ppp WHERE ppp.po = b.po AND ppp.product = dp.product LIMIT 1 ) AS harga, ( SELECT pp.cur_amount FROM len_enq_po_product pp WHERE pp.po = b.po AND pp.product = dp.product LIMIT 1 ) AS total, ( SELECT p.tgl_po FROM len_enq_po p WHERE b.po = p.po LIMIT 1 ) AS tanggal_po, ( SELECT p.tgl_habis_kontrak FROM len_enq_po p WHERE b.po = p.po LIMIT 1 ) AS tanggal_kirim, IF (( SELECT op.amendment FROM len_option_po op WHERE op.po = b.po LIMIT 1 ) IS TRUE, 'YA', 'TIDAK' ) AS eta, ( SELECT bap.tgl_print FROM len_enq_bapb bap WHERE bap.po = b.po AND bap.product = dp.product LIMIT 1 ) AS barang_tiba, IF (( SELECT op.negotiation FROM len_option_po op WHERE op.po = b.po ) IS TRUE, 'YA', 'TIDAK' ) AS negosiasi, ( SELECT ( dp.cur_amount - pp.cur_amount ) FROM len_enq_po_product pp WHERE pp.po = b.po LIMIT 1 ) AS efisiensi, ( SELECT bap.bapb FROM len_enq_bapb bap WHERE bap.po = b.po AND bap.product = dp.product LIMIT 1 ) AS bapb, ( SELECT bap.qty FROM len_enq_bapb bap WHERE bap.po = b.po AND bap.product = dp.product LIMIT 1 ) AS jml_brg, '' AS 'status', '' AS 'keterangan' FROM len_enq_dpb d JOIN len_enq_dpb_product dp ON dp.dpb = d.dpb LEFT JOIN len_enq_spph_product b ON ( b.dpb = dp.dpb AND b.product = dp.product ) WHERE LEFT (d.tgl_pengajuan, 10) BETWEEN '" + from + "' AND '" + to + "' ORDER BY d.tgl_pengajuan ASC";
